...
// UpdateStage lets callers manually set a candidate's stage.
...
func ListLegacy(c *gin.Context) {
	if pool != nil {
		result, err := fetchCandidates(c.Request.Context(), "")
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
			return
		}
	...
	mu.RLock()
	defer mu.RUnlock()
	result := make([]Candidate, len(memCandidates))
	copy(result, memCandidates)
	sort.Slice(result, func(i, j int) bool {
